...
export default function AiCourseScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    ...
    <ScrollView ref={scrollRef} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      <PageHero
        description="날짜, 충남 지역, 인원과 여행 스타일을 바탕으로 나에게 꼭 맞는 코스를 구성해드려요."
        eyebrow="맞춤 여행 코스"
      ...
      {error ? <View accessibilityLiveRegion="polite" style={styles.errorBox}><Ionicons color={CourseColors.error} name="alert-circle-outline" size={22} /><View style={styles.errorContent}><Text style={styles.errorTitle}>추천 코스를 만들지 못했어요</Text><Text style={styles.error}>{error}</Text><Text style={styles.errorHint}>입력한 조건은 그대로 유지되어 있어요. 잠시 후 다시 시도해 주세요.</Text></View></View> : null}
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: CourseColors.primary }, container: { flexGrow: 1, backgroundColor: CourseColors.background, paddingBottom: 50 }, body: { width: '100%', maxWidth: PAGE_LAYOUT.desktopMaxWidth, alignSelf: 'center', paddingHorizontal: 18, marginTop: PAGE_LAYOUT.sectionSpacing, gap: 24 }, collapsedForm: { display: 'none' },
  errorBox: { alignItems: 'flex-start', backgroundColor: '#FFF9F6', borderWidth: 1, borderColor: '#E6BDB3', borderRadius: 14, flexDirection: 'row', gap: 9, padding: 13 }, errorContent: { flex: 1, gap: 3 }, errorTitle: { color: CourseColors.text, fontSize: 14, fontWeight: '900' }, error: { color: CourseColors.error, fontSize: 13, lineHeight: 18 }, errorHint: { color: CourseColors.muted, fontSize: 12, lineHeight: 17, marginTop: 2 },
  result: { gap: 16, backgroundColor: CourseColors.white, borderRadius: 18, borderWidth: 1, borderColor: CourseColors.border, padding: 16 }, resultHeading: { flexDirection: 'row', alignItems: 'center', gap: 10 }, sparkle: { width: 42, height: 42, borderRadius: 21, backgroundColor: CourseColors.primarySoft, alignItems: 'center', justifyContent: 'center' }, sparkleText: { color: CourseColors.primary, fontSize: 20, fontWeight: '900' }, resultTitleWrap: { flex: 1 }, eyebrow: { color: CourseColors.primary, fontWeight: '900', fontSize: 12, marginBottom: 3 }, title: { fontSize: 21, lineHeight: 28, fontWeight: '900', color: CourseColors.text }, editButton: { borderColor: CourseColors.border, borderRadius: 12, borderWidth: 1, minHeight: 44, justifyContent: 'center', paddingHorizontal: 11 }, editButtonText: { color: CourseColors.primaryDark, fontSize: 14, fontWeight: '800' }, conditionSummary: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, conditionChip: { backgroundColor: CourseColors.background, borderColor: CourseColors.border, borderRadius: 10, borderWidth: 1, color: '#6B5730', fontSize: 13, fontWeight: '700', overflow: 'hidden', paddingHorizontal: 9, paddingVertical: 6 }, resultSection: { gap: 12 }, resultSectionTitle: { color: CourseColors.text, fontSize: 18, fontWeight: '900' },
