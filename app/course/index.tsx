...
export default function CourseListScreen() {
  ...
  const isDesktop = width >= PAGE_LAYOUT.desktopBreakpoint;
        ...
        <View style={styles.body}>
          ...
          {!loading && !error && courses.length === 0 ? <View style={styles.stateCard}>
            ...
            <View style={styles.actions}>
              ...
              <Pressable style={styles.aiButton} onPress={() => router.push('/course/ai')}><Text style={styles.aiButtonText}>AI 코스 만들기</Text><Text style={styles.aiArrow}>→</Text></Pressable>
            </View>
          </View> : null}
          {!loading && !error && courses.length > 0 ? <View style={styles.listSection}>
            <View style={styles.sectionHeading}><View style={styles.sectionTitleWrap}><Text style={styles.sectionTitle}>저장된 코스 <Text style={styles.sectionCount}>{courses.length}</Text></Text></View><Pressable accessibilityLabel="AI 코스 만들기" style={({ pressed }) => [styles.smallAiButton, pressed && styles.smallAiButtonPressed]} onPress={() => router.push('/course/ai')}><Ionicons color={CourseColors.white} name="add" size={17} /><Text style={styles.smallAiText}>AI 코스 만들기</Text></Pressable></View>
            <View style={[styles.controls, isDesktop && styles.controlsDesktop]}>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.filterRow, isDesktop && styles.filterRowDesktop]} style={styles.filterScroll}>
...
function FilteredEmptyState({ filter, onAiPress }: { filter: CourseFilter; onAiPress: () => void }) {
  ...
  return <View style={styles.filteredEmpty}>
    ...
    {isAi ? <Pressable accessibilityRole="button" onPress={onAiPress} style={({ pressed }) => [styles.filteredEmptyAction, pressed && styles.controlPressed]}><Text style={styles.filteredEmptyActionText}>AI 코스 만들기</Text></Pressable> : null}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  listSection: { gap: 18 }, sectionHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, sectionTitleWrap: { flexShrink: 1 }, sectionTitle: { color: CourseColors.text, fontSize: 21, fontWeight: '900' }, sectionCount: { color: CourseColors.primary, fontSize: 17, fontWeight: '900' }, smallAiButton: { alignItems: 'center', backgroundColor: CourseColors.primary, borderRadius: 12, flexDirection: 'row', gap: 6, minHeight: 44, paddingHorizontal: 14 }, smallAiButtonPressed: { backgroundColor: CourseColors.primaryDark, opacity: 0.9 }, smallAiText: { color: CourseColors.white, fontWeight: '900', fontSize: 14 },
  controls: { gap: 10, position: 'relative', zIndex: 100 }, controlsDesktop: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, filterScroll: { flexGrow: 0 }, filterRow: { gap: 7, paddingRight: 4 }, filterRowDesktop: { paddingRight: 16 }, filterChip: { alignItems: 'center', backgroundColor: '#FFFCF7', borderColor: CourseColors.border, borderRadius: 10, borderWidth: 1, flexDirection: 'row', gap: 4, minHeight: 38, paddingHorizontal: 13 }, filterChipSelected: { backgroundColor: CourseColors.primary, borderColor: CourseColors.primary }, filterChipText: { color: CourseColors.text, fontSize: 13, fontWeight: '800' }, filterChipTextSelected: { color: CourseColors.white }, controlPressed: { opacity: 0.72 }, statusFilterWrap: { alignItems: 'flex-end', position: 'relative', zIndex: 101 }, statusFilterButton: { alignItems: 'center', alignSelf: 'flex-end', backgroundColor: CourseColors.white, borderColor: CourseColors.border, borderRadius: 10, borderWidth: 1, flexDirection: 'row', gap: 5, minHeight: 40, paddingHorizontal: 12 }, statusFilterButtonText: { color: CourseColors.primaryDark, fontSize: 13, fontWeight: '800' }, statusFilterMenu: { backgroundColor: CourseColors.white, borderColor: CourseColors.border, borderRadius: 12, borderWidth: 1, elevation: 12, minWidth: 158, overflow: 'hidden', position: 'absolute', right: 0, shadowColor: '#40382C', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.14, shadowRadius: 12, top: 45, zIndex: 102 }, statusFilterOption: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 42, paddingHorizontal: 12 }, statusFilterOptionText: { color: CourseColors.text, fontSize: 13, fontWeight: '700' }, statusFilterOptionSelected: { backgroundColor: CourseColors.primarySoft }, statusFilterOptionTextSelected: { color: CourseColors.primaryDark, fontWeight: '900' },
  list: { alignItems: 'stretch', columnGap: 10, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', position: 'relative', rowGap: 10, zIndex: 1 },
  filteredEmpty: { alignItems: 'center', backgroundColor: '#FFFCF7', borderColor: CourseColors.border, borderRadius: 16, borderWidth: 1, gap: 7, paddingHorizontal: 18, paddingVertical: 24 }, filteredEmptyTitle: { color: CourseColors.text, fontSize: 16, fontWeight: '900', marginTop: 2, textAlign: 'center' }, filteredEmptyDescription: { color: CourseColors.muted, fontSize: 14, lineHeight: 20, textAlign: 'center' }, filteredEmptyAction: { borderColor: CourseColors.primary, borderRadius: 12, borderWidth: 1, justifyContent: 'center', marginTop: 5, minHeight: 44, paddingHorizontal: 15 }, filteredEmptyActionText: { color: CourseColors.primaryDark, fontSize: 14, fontWeight: '900' },
});
