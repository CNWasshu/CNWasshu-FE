...
export function AiCourseForm({ loading, onSubmit, onValidationError }: Props) {
  ...
  return <View style={styles.form}>
    <View style={styles.formHeading}><View style={styles.formIcon}><Ionicons color={CourseColors.primary} name="sparkles-outline" size={18} /></View><View><Text style={styles.sectionTitle}>기본 조건</Text><Text style={styles.sectionDescription}>원하는 여행의 기본 정보를 알려주세요.</Text></View></View>
    ...
    <Pressable accessibilityRole="button" disabled={loading} onPress={submit} style={({ pressed }) => [styles.button, loading && styles.loadingButton, pressed && !loading && styles.buttonPressed]}>{loading ? <ActivityIndicator color={CourseColors.white} size="small" /> : null}<Text style={styles.buttonText}>{loading ? 'AI가 코스를 만들고 있어요' : '추천 코스 만들기'}</Text>{!loading ? <Ionicons color={CourseColors.white} name="arrow-forward" size={18} /> : null}</Pressable>
  </View>;
}
  ...
  return <View style={styles.field}>
    ...
    <View style={[styles.regionShell, error && styles.errorShell]}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.regionList} keyboardShouldPersistTaps="handled">
      {CHUNGNAM_REGIONS.map((region) => {
        ...
        return <Pressable key={region} accessibilityRole="radio" accessibilityState={{ checked: selected }} hitSlop={3} onPress={() => onChange(region)} style={[styles.regionChip, selected && styles.regionChipSelected]}>
...
function Choice<T extends string>({ label, options, value, onChange }: { label: string; options: { label: string; value: T }[]; value: T; onChange: (value: T) => void }) {
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><View accessibilityRole="radiogroup" style={styles.choices}>{options.map((option) => <Pressable key={option.value} accessibilityRole="radio" accessibilityState={{ checked: value === option.value }} hitSlop={2} onPress={() => onChange(option.value)} style={[styles.choice, value === option.value && styles.selected]}>{value === option.value ? <Ionicons color={CourseColors.white} name="checkmark" size={16} /> : null}<Text style={[styles.choiceText, value === option.value && styles.selectedText]}>{option.label}</Text></Pressable>)}</View></View>;
}
const styles = StyleSheet.create({
  form: { backgroundColor: CourseColors.white, borderRadius: 18, padding: 16, gap: 15, borderWidth: 1, borderColor: CourseColors.border },
  formHeading: { flexDirection: 'row', alignItems: 'center', gap: 10 }, formIcon: { width: 34, height: 34, borderRadius: 17, backgroundColor: CourseColors.primarySoft, alignItems: 'center', justifyContent: 'center' }, sectionTitle: { fontSize: 18, fontWeight: '900', color: CourseColors.text }, sectionDescription: { color: CourseColors.muted, fontSize: 12, marginTop: 1 },
  fieldGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, oneColumn: { flexDirection: 'column' }, field: { gap: 7 }, label: { color: CourseColors.text, fontWeight: '800', fontSize: 13 },
  regionGuide: { color: CourseColors.muted, fontSize: 12 }, regionShell: { borderColor: 'transparent', borderRadius: 12, borderWidth: 1, marginHorizontal: -3, paddingLeft: 3 }, regionList: { gap: 6, paddingRight: 16 }, regionChip: { alignItems: 'center', flexDirection: 'row', gap: 3, minHeight: 38, paddingHorizontal: 11, borderWidth: 1, borderColor: CourseColors.border, borderRadius: 999, backgroundColor: CourseColors.background }, regionChipSelected: { borderColor: CourseColors.primary, backgroundColor: CourseColors.primary }, regionChipText: { color: '#6B5730', fontSize: 12, fontWeight: '700' }, regionChipTextSelected: { color: CourseColors.white, fontWeight: '800' },
  stepper: { alignItems: 'center', alignSelf: 'flex-start', backgroundColor: CourseColors.background, borderColor: CourseColors.border, borderRadius: 14, borderWidth: 1, flexDirection: 'row', minHeight: 44 }, stepperButton: { alignItems: 'center', justifyContent: 'center', minHeight: 44, minWidth: 44 }, stepperDisabled: { opacity: 0.35 }, peopleValue: { color: CourseColors.text, fontSize: 15, fontWeight: '900', minWidth: 58, textAlign: 'center' },
  choices: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, choice: { alignItems: 'center', flexDirection: 'row', gap: 4, minHeight: 40, borderWidth: 1, borderColor: CourseColors.border, backgroundColor: CourseColors.background, borderRadius: 999, paddingHorizontal: 13 }, selected: { borderColor: CourseColors.primary, backgroundColor: CourseColors.primary }, choiceText: { color: '#6F6558', fontSize: 12, fontWeight: '700' }, selectedText: { color: CourseColors.white, fontWeight: '800' },
  errorShell: { backgroundColor: '#FFF9F6', borderColor: '#E6BDB3' }, errorRow: { alignItems: 'center', flexDirection: 'row', gap: 6 }, errorText: { color: CourseColors.error, flex: 1, fontSize: 12, lineHeight: 18 }, button: { minHeight: 50, backgroundColor: CourseColors.primary, borderRadius: 16, paddingHorizontal: 18, paddingVertical: 13, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }, loadingButton: { backgroundColor: '#67976C' }, buttonPressed: { backgroundColor: CourseColors.primaryDark }, buttonText: { color: CourseColors.white, fontSize: 15, fontWeight: '900' },
});
